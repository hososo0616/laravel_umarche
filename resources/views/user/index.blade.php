<x-app-layout>
    <x-slot name="header">
        ...
        <form action="{{ route('user.items.index') }}" method="GET">
            <div class="lg:flex lg:justify-between items-around">
                <div class="lg:flex items-center">
                    ...
                    <div class="flex space-x-2 items-center">
                        <div><input name="keyword" placeholder="キーワードを入力" class="border border-gray-500 py-2"></div>
                        <div><button
                    ...
                    <div>
                        ...
                        <select id="sort" name="sort" class="mr-4">
                            ...
                            <option value="{{ \Constant::SORT_ORDER['later'] }}"
                                @if (\Request::get('sort') === \Constant::SORT_ORDER['later']) selected @endif>最新
                            </option>
                            <option value="{{ \Constant::SORT_ORDER['older'] }}"
                                @if (\Request::get('sort') === \Constant::SORT_ORDER['older']) selected @endif>古い
                            </option>
                        </select>
                    </div>
                    <div>
                        ...
                        @foreach ($products as $product)
                            <div class="w-1/4 p-2 md:p-4">
                                <a href="{{ route('user.items.show', ['item' => $product->id]) }} ">
                                    <div class="border rounded-mdp-2 md:p-4">
                                        ...
                                        <div class="mt-4">
                                            ...
                                            <p class="mt-1">{{ number_format($product->price) }}<span
                                                    class="text-sm to-gray-700">円（税込）</span></p>
                                        </div>
                                    </div>
                                </a>
                            </div>
